...
def test_geographic_position_initialization():
    ...
    geo_pos = GeographicPosition(
        ...
        altitude=100.0,
        earth_ellispoid_model=earth_model
    )
    ...
    assert geo_pos.earth_ellispoid_model == earth_model
...
def test_geographic_position_str_and_repr():
    ...
    assert (
        str(geo_pos)
        == f"GeographicPosition(latitude={math.radians(52.2296756)}, longitude={math.radians(21.0122287)}, altitude=100.0)"
    )
    assert (
        repr(geo_pos)
        == f"GeographicPosition(latitude={math.radians(52.2296756)}, longitude={math.radians(21.0122287)}, altitude=100.0)"
